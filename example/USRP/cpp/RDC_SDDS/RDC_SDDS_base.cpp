...
RDC_SDDS_base::RDC_SDDS_base(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl) :
...
RDC_SDDS_base::RDC_SDDS_base(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, char *compDev) :
...
RDC_SDDS_base::RDC_SDDS_base(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities) :
...
RDC_SDDS_base::RDC_SDDS_base(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities, char *compDev) :
...
*******************************************************************************************/
void RDC_SDDS_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    frontend::FrontendTunerDevice<frontend_tuner_status_struct_struct>::start();
    ThreadedComponent::startThread();
}

void RDC_SDDS_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
...
void RDC_SDDS_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void RDC_SDDS_base::loadProperties()
{
...
CF::Properties* RDC_SDDS_base::getTunerStatus(const std::string& allocation_id)
{
...
void RDC_SDDS_base::frontendTunerStatusChanged(const std::vector<frontend_tuner_status_struct_struct>* oldValue, const std::vector<frontend_tuner_status_struct_struct>* newValue)
{
    this->tuner_allocation_ids.resize(this->frontend_tuner_status.size());
}

void RDC_SDDS_base::assignListener(const std::string& listen_alloc_id, const std::string& allocation_id)
{
...
void RDC_SDDS_base::removeListener(const std::string& listen_alloc_id)
{
    if (listeners.find(listen_alloc_id) != listeners.end()) {
        listeners.erase(listen_alloc_id);
    }
}
void RDC_SDDS_base::removeAllocationIdRouting(const size_t tuner_id) {
}
